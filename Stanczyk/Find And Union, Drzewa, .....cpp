...
struct CoverITree{
	...
	CoverITree( int size ){ 
		...
		minstat = new PII[s];
		sqr = new int[s];
		REP(x,s) el[x] = ma[x] = sum[x] = sqr[x] = 0;
		FOR(x,s/2,s-1){
	...
	void Mark( int wp, int wk, int g ){
		...
		if( p <= wp && k >= wk ){
			...
			sqr[g] = sqr[2*g] + sqr[2*g+1];
			sqr[g] += 2*el[g]*su + ( wk-wp )*el[g]*el[g];
		}		
		ma[g] = el[g] > 0 ? wk - wp : ( wp == wk-1 ? 0 : ma[2*g] + ma[2*g + 1] );
	...
	int Squares( LL wp, LL wk, LL g, LL sumka ){
...
/*	// FAU
	...
	REP(x,m){
		...
		cin >> e1 >> e2;
		fau.Union(e1,e2);
		cout << "Z³¹czenie zbiorów zawierajacych elementy " << e1 << " i " << e2 << endl;
...
/*	// MaxTree
	int w1,w2,w3, n;
	//wczytaj maxymalny zakres elementow, z ktorego chcesz wyznaczac maxy
	cin >> n;
	MaxTree tree( (int)ceil( log2(n) ) );
	// wykonaj liste operacji
	while( cin >> w1 >> w2 >> w3 ){		
		if( w1 == 0 ){
			// operacja zmiant wartosci klucza
			tree.Set( w2,w3 );
			cout << "Zmiana wartosci elementu " << w2 << " na " << w3 << endl;
		}
		else{
			// operacja wypisania maximum na przedziale [w2...w3]
			cout << "Maximum na przedziale " << w2 << "..." << w3 << " = " 
				<< tree.Find( w2,w3 ) << endl;
		}
		
		cout << endl << "Oto twoje wartosci elementow z drzewa maximow:" << endl << "\t";
		REP(i,n) cout << "i = " << i << "   el["<<i<<"] = " << tree.el[ i + tree.s ] << endl;
		cout << endl;
	}
	return 0;*/
...
/*	// CountTree
	int w1,w2,w3, n;
	//wczytaj maxymalny zakres elementow, z ktorego chcesz wyznaczac sumy
	cin >> n;
	CountTree tree( (int) ceil( log2(n) ) );
	// wykonaj liste operacji
	while( cin >> w1 >> w2 >> w3 ){		
		if( w1 == 0 ){
			...
			// wyznaczenie sumy na przedziale [w2...w3]
			cout << "Suma na przedziale " << w2 << "..." << w3 << " = " << tree.Find( w2,w3 ) << endl;
		}
	}
	return 0;*/
...
/*	// PosTree
	...
	//wczytaj maxymalny zakres elementow, z ktorego chcesz wyznaczac sumy
	cin >> n;
	PosTree tree( (int) ceil( log2(n) ) );
	cout << "tree.s = " << tree.s << endl;
	
	while( cin >> w1 >> w2 ){
		if( w1 == 0 ){
			cin >> w3;
			tree.Add(w2,w3);
			cout << "Do drzewa dodano/usunieto " << w3 << " element(ow) " << w2 << endl;
		}
		else{
			cout << w2 << "-statystyka pozycyjne(" << w2 << "-ty najmniejszy element) = " << tree.Find( w2 );
		}
		
		cout << endl << "Oto twoje wartosci elementow z drzewa pozycyjnego:" << endl << "\t";
		REP(i,n) cout << "i = " << i << "   el["<<i<<"] = " << tree.el[ i + tree.s ] << endl;
		cout << endl;
	}
	return 0;*/
...
/*	// CoverTree
	...
	cin >> n;
	CoverTree tree( (int) (ceil( log2(n) ) + 0.1) );
	cout << "tree.s = " << tree.s << endl;
	// wczytaj polecenia...
	while( cin >> w1 >> w2 >> w3 ){
		if( w1 == 0 ){
	...
	// wczytaj zakres liczb dla ktorego chce stworzyc drzewko pokryciowe
	cin >> n;
	CoverITree tree( (int) (ceil( log2(n) ) ) );

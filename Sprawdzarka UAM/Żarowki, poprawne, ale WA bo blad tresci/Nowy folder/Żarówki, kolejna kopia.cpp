...
class Node{
public:
	Node() : hasLeafSon(false), isLeaf(false) {}
	int on;
	int lighted;
	int off;
...
int getOnStatus( int parent, int myNumber ){
	...
	for( int i=0; i<V[myNumber].size(); i++ ){
		if( V[myNumber][i] != parent ){
			m += min( nodes[ V[myNumber][i] ].min, nodes[ V[myNumber][i] ].off );			// UWAGA, TUTAJ ROZNE WERSJE !!!
		}
	}
...
void DFS( int parent, int myNumber ){
	...
	if( V[myNumber].size() == 0 || ( V[myNumber].size() == 1 && parent != myNumber  )   ){ // jesli jest lisciem
		...
		for( int i=0; i<V[myNumber].size(); i++ ){
			if( ifWas[ V[myNumber][i] ] == false && V[myNumber][i] != parent ){
		...
		if( n.hasLeafSon ){
		//	n.isLeaf = false; // to nie jest potrzebne, bo jesli jeslesmy w elsie to wiadomo, ze nie jest lisciem
			off = infinity;
		} 
		n.on = on;
		n.lighted = lighted;

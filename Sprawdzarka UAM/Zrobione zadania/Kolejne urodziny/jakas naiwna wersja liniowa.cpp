...
/*	for( int i=0; i<V.size(); i++ ){
	...
	while( true ){
		while( square( current + wektory[beg] ) >= square( current ) && beg!=end ){
			current = current + wektory[beg];
			beg++;
			beg %= wektory.size();
		}
		beg++;
		...
		while( square( current - wektory[end] ) >= square( current ) && beg!=end ){
			current = current - wektory[end];
			end++;
			end %= wektory.size();
		}
		end++;

...
//	if( index != parent ){
		...
		REP( i,g[index].size() ){
			...
			if( g[index][i] != parent && ( tab[ g[index][i] ] == true || g[ g[index][i] ].size() >= 3 )  ) il++; // czy tutaj to sprawdzenie rozmiaru powinno byc?
		}
//	}
...
//	else if( g[index].size() >= 4 ) ile[index]++;
...
//	bool wKolko = false;
	...
	while(l_zest--){
		...
		REP(i,N) if( g[i].size() == 1 ){ start = i; break;		}
		
	//	cout << endl << "Wierzcholek startowy to:  " << start+1 << endl;
		backDFS( start, start );
		il = 0;
		REP( i,g[start].size() ){
			if( tab[ g[start][i] ] == true || g[ g[start][i] ].size() >= 3 ){
				il++;
			}
		}
//		
		if( g[start].size() >= 4 ) il++;
	...
	if( wypisac ){	cout << "Tablica ile po backDFS przed forDFS:" << endl;
		REP( i,N ) cout << i+1 << ":  " << ile[i] << endl;
		
		cout << "Stan tablicy tab po backDFS przed forDFS:" << endl;
		REP(i,N) cout << i+1 << ":  " << tab[i] << endl;
		cout << endl;
		cout << "Stan tablicy tab2 po backDFS przed forDFS:" << endl;
		REP(i,N) cout << i+1 << ":  " << tab2[i] << endl;
		cout << endl;
		cout << "Stan tablicy tab3 po backDFS przed forDFS:" << endl;
	...
	if( wypisac ){
		...
		cout << "Stan tablicy tab po forDFS przed forawrdDFS:" << endl;
		REP(i,N) cout << i+1 << ":  " << tab[i] << endl;
		cout << endl;
		cout << "Stan tablicy tab2 po forDFS przed forawrdDFS:" << endl;
		REP(i,N) cout << i+1 << ":  " << tab2[i] << endl;
		cout << endl;
		cout << "Stan tablicy tab3 po forDFS przed forawrdDFS:" << endl;
		REP(i,N) cout << i+1 << ":  " << tab3[i] << endl;
		cout << endl;
	}
		forwardDFS( 0,0 );
	...
	if( wypisac ){	cout << "Oto graf indukowany przez niepoprawne krawdzie:" << endl;
		writeGraph( induced );}
	...
	//	writeDFS(0,0,true);
		...
		if( wypisac ){	cout << "Oto twoja sciezka:"<< endl;

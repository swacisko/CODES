...
int main(){
	...
	/*		if( ile_zajetych[ index ] == V[index].size() ){ // jesli wszystkie krawedzie sa zajete
				priorytety[index].ST = 0; // nie bedziemy wiecej potrzebowac juz tego wierzcholka, skoro ma wszystkie krawedzie zajete
	...
	//					cout << endl << "Wszystkie krawedzie dla danego weirzcholka sa juz zajete"<< endl;
				...
				if( ile_zajetych[ index ] == V[index].size() ){ // jesli wszystkie krawedzie sa zajete to szukam nastepnego o najwyzszym priorytecie
					max2 = 0;
					index2 = 0;
					
					priorytety[index].ST = 0;
					REP(k,N){
						if( k != index && priorytety[k].ST > max2 ){
							max2 = priorytety[k].ST;
							index2 = k;
						}
					}
				}

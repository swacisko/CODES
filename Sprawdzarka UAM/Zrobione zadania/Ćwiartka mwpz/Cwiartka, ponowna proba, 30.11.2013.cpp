...
int main(){
	...
	if( wypisac )		cout << endl << "Malgosia zajmuje krawedz nr " << mapa.find( MP(zagrania[i].ST,zagrania[i].ND  ) )->ND+1 << endl;
			
			ile_zajetych[ zagrania[i].ST ]++;
			ile_zajetych[ zagrania[i].ND ]++;
			
	if( wypisac )		REP(j,N) cout << "ile_zajetych["<<j+1<<"] = "<< ile_zajetych[j] << "  ile_zajetych_przez_jasia["<<j+1<<"] = " << ile_zajetych_przez_jasia[j] << endl;
	...
	if( wypisac ){	cout << endl << "\tWierzcholek o najwiekszym zagrozeniu:";
			wypiszWierzcholek(index);
			
			cout << "Oto maxy:  rozmiar --> " << maxy.size() << endl;
			REP(i,maxy.size()){
				cout << maxy[i]+1 << " ";
			}
			cout << endl << endl; }
		...
		if( ! wypisac )				cout << d+1 << " ";
		if( wypisac )			cout << endl << "Jas zajmuje krawedz " << d+1 << endl;
						wypisal = true;
	//					cout << endl << "Zajmuje krawedz "<< d+1 << ":  "<< vec[d].ST+1 << " "<< vec[d].ND+1 << endl;
		...
		if( !wypisac )				cout << k+1 << " ";
		if( wypisac )			cout << endl << "HAHAHA, JAs zajmuje krawedz " << k+1 << endl;

...
//	cout << "Skopiowalem" << endl;
	...
	if( S[1]==1 && S[2]==1 && S[3]==1 && S[4]==1 && S[5]==1 ) return Strit;
...
int main(){
	...
	/*	REP( k,rows.size() ){
		...
		ok = true;		
		REP(i,5){ // tutaj sprawdzam, czy oby na pewno jest 5 permutacji liczb od 1 do 5
			REP(k,6) tablica[k] = false;
			REP(k,5){
				tablica[  change[ card_column[i][k].column_number ]  ] = true;
			}
			
			REP(k,5){
				if( tablica[k+1] == false ) {
					ok = false;
				//	cout << tablica[k] << " == false" << endl;
					break;
				}
			}
		...
		if( ok ){
			REP( i,5 ){
				REP( k,5 ){
					if( i*k != 16 ) cout << change[ card_column[i][k].column_number ] << " ";
					else cout << change[ card_column[i][k].column_number ];
				}
			}
			cout << endl;
		}
		else{
		//	cout << endl << endl << "COS JEST BARDZO ZLE, NIE POWINNO MNIE TUTAJ BYC..." << endl << endl;
			while(true);
			REP(i,5) perm[i] = i+1;
		
			REP(i,5){
				REP( k,5 ){
					if( i*k != 16 ) cout << perm[k] << " ";
					else cout << perm[k];
				}
				
				REP(t,ile_razy) next_permutation( perm, perm+5 );
			}
			cout << endl;
			ile_razy = ile_razy + 7;

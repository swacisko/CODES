...
void deletePath(tile * pole){
	tile * wsk = pole;
	do{
		pole->onPath = false;
		pole = pole->par;
	} while( pole != 0 && pole != wsk );
	pole = wsk;
}


// funkcja przyjmuje wskaznik na jakies pole oraz wskaznik na kierunek, Z KTOREGO PRZYSZEDLEM do danego pola
bool dfs( tile* pole, int from, char let ){
WAR	ILE++;
WAR	REP(i,ILE) cout << "\t";
WAR	cout << "Jestem w dfs w plytce (" << pole->x << "," << pole->y << ")    from = " << from << "   " << ( from == NORTH?"NORTH":( from==EAST?"EAST":(from==SOUTH?"SOUTH":"WEST") ) ) << endl;
...
//WAR	if( pole->par != 0 ) cout << "Jestem w dfs w plytce (" << pole->x << "," << pole->y << ")   par = " << pole->par->x << "," << pole->par->y << endl;
	...
	REP( i,4 ){ // najpierw wywoluje dfs'y dla kazdego, w tej petli tylko dla tych pol, ktore sa pod katem prostym
		if( from > 5 || ( i != from && i != ( from+2 )%4 ) ){
			while( pole->tab[i] != 0 && (pole->tab[i]->par = pole) && ( pole->next = pole->tab[i] ) && dfs( pole->tab[i], (i+2)%4, ( let=='A'?'B':'A' ) ) ){				
WAR					ILE--;
					deletePath(pole);
					return true;
			}
...
WAR	REP( i,SIZE(tab1) ){
	...
	REP( i,SIZE(tab1) ){
		ind = 10;
		while( tab1[i]->letter != 'A' && tab1[i]->letter != 'B' ) dfs( tab1[i], ind++, 'A' );
	}

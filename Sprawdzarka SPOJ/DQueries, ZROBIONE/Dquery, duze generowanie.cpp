...
void getSecondAnswer( int p, int q ){
	bool *is = new bool[WART+2];
	REP(x,WART+2) is[x] = false;
	int ile = 0;
	FOR(i,p-1,q-1){
		if( !is[ arr[i] ] ){
			ile++;
...
int main(){
	...
	N = 50000;
		...
		CountTree tree( (int)(ceil( log2(N) ) + 0.1)  );
	...
	//	leftMost = new int[N];
		...
		REP( i,1000010 ) M[i] = true;
		REP( i,N ) M[ arr[i] ] = false;		
		REP(i,N) marked[i] = false;
		REP( i,1000010 ) leftMost[i] = -1;
		REP(i,N){
			if( !M[ arr[i] ] ){ 
				M[ arr[i] ] = true;
				marked[i] = true;

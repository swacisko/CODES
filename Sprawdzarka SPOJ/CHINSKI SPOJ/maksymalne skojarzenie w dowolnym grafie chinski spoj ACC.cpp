...
int main(){
	...
	while(true){	
		cin >> N >> K;
		if( N == 0 && K == 0 ) break;
		G = VVI(2*N); // uwaga! tutaj musi byc graf G, bo graf V jest globalny i uzywany w przeplywach!!!
		VI IQ(N);
		REP(i,N){
			cin >> a;
			IQ[i] = a;
		...
		int t;
		cout << (t = MaxMatch()) / 2 << endl;
		
		//	cout << "Maksymalne skojarzenie posiada " << t << " par." << endl;
		//	REP(i,SIZE(matching)) if( matching[i] == -1 ) cout << "Wierzcholek " << i+1 << " nie jest skojarzony" << endl;
		//							else cout << "Wierzcholek " << i+1 << " jest skojarzony z " << matching[i]+1 << endl;
			
			
	//		REP(i,SIZE(matching)) cout << matching[i]  + 1 << " ";

...
//	ios_base::sync_with_stdio(0);
	...
	while( l_zest-- ){
		...
		vector<vector<BigNum> > ile( SIZE(X)+1, vector<BigNum>( SIZE(Z)+1 ) );
		
		REP(i,SIZE(ile) ) REP(k,SIZE(ile[i])) ile[i][k] = 0;
		
	//	cout << "SIZE(Z) = " << SIZE(Z) << "    SIZE(X) = " << SIZE(X) << endl;
		
		 REP(i,SIZE(X)+1 ) ile[ i ][ SIZE(Z) ] = 1;
		
		FORD( i,SIZE(X),1 ) FORD( j,SIZE(Z),1 ){
			ile[i-1][j-1] = ile[i][j-1];
			if( X[i-1] == Z[j-1] ){
			//	cout << "\t\tX["<<i-1<<"] == Z["<<j-1<<"]" << endl;
				ile[i-1][j-1] += ile[i][j];
			//	cout << "\t\t\tile["<<i-1<<"]["<<j-1<<"] = "; ile[i-1][j-1].write(); cout << endl;
			}			
		}
		
	/*	REP(i,SIZE(ile)){
			cout << i << ":  " << endl;
			REP( j,SIZE(ile[i]) ) { cout << "\t" << j << ":  "; ile[i][j].write(); cout << endl; }
		}*/

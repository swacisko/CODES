...
public:
	...
	void execute(){
		...
		switch( which_case ){
			case 0:{
				if( round_nr == 0 && id == 0 ) sendMessage( neighbours[0],id_los );
				round_nr++;
				
				notif nf = getLatestMessage( neighbours[1] );
				if( nf.liczba == notif::BLANK ){
					break;
				}
				
				if( round_nr > number_of_vertices+1 ){
					if( round_nr > 2*number_of_vertices + 1 ){
						cout << "NIC SIE NIE POWINNO JUZ DZIAC, number of vertices = " << number_of_vertices << endl;
					}
					else{
						suma_id = nf.liczba;
						sendMessage(neighbours[0],suma_id);
					}
					break;
				}
				else{
					suma_id = nf.liczba;
					sendMessage( neighbours[0], suma_id + id_los );
				}
	...
	void writeState(){
		cout << "Jestem wierzcholkiem o id = " << id << "  id_los = " << id_los << "  suma_id = " << suma_id << endl;

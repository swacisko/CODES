...
public:
	...
	void execute(){
		...
		switch( which_case ){
			case 0:{
				if( round_nr == 0 && id == START ){
					REP(i,SIZE(neighbours)) sendMessage(neighbours[i]);
					cout << "id = 0  wysylam wiadomosci" << endl;
					which_case++;
				}
				...
				REP(i,SIZE(neighbours)){
					...
					if( nf.liczba != notif::BLANK ){
						par_id_los = nf.from;
						cout << "id = " << setw(2) << id << "  id_los = " << setw(3) << id_los << "   moim rodzicem w BFS jest w. o id = ";
						REP(k,number_of_vertices) if( (*graph)[k].id_los == nf.from){ cout << (*graph)[k].id << flush; break;			} // UWAGA TUTAJ	
						// JEST NARUSZENIE ZASADY, ABY NIE ODWOLYWAC SIE DO MIENNYCH GLOBALNYCH, ALE TYLKO W CELU WIZUALIZACJI, A NIE W CELU DZIALANIA ALGORYTMU
						cout << " id_los = " << setw(3) << nf.from << endl;
						
						REP(k,SIZE(neighbours)) sendMessage( neighbours[k] );
						which_case++;
						break;
					}
				}
	...
	// getProperColor()
	void writeState(){
		cout << "Jestem wierzcholkiem o id = " << setw(2) << id << "  id_los = " << setw(3) << id_los << "   par_id_los = " << setw(3) << par_id_los << endl;		
	}
	
	// funkcja wysyla sasiadowi nr sock wiadomosc mess
	void sendMessage( int sock ){
		(*graph_copy)[ sock ].kom[id].PB( notif( 0,id_los ) ); // dodaje wiadomosc do nowej wersji grafu, wersji grafu z nastepnej rundy
	}
...
int main(){
	...
	if( s == "TAK" || s == "tak" ){
		...
		do{
			cout << endl << "Z ktorego wierzcholka chcesz zaczac budowac drzewo BFS?" << endl;
			cin >> START;
		}
	...
	cout << endl << "Drzwo buduje zaczynajac od wierzcholka " << START << endl << endl;

...
namespace TSP{
	...
	double dist( PTT & p, PTT & q ){
		return ceil( sqrt( (p.ST-q.ST)*(p.ST-q.ST) + (p.ND-q.ND)*(p.ND-q.ND) ) );
		//return (int) ceil( sqrt( (p.ST-q.ST) + (p.ND-q.ND) ) ); 
	}
	...
	double getDist( int citya, VI & frag, int cityz, const double & _mindist ){
		if( frag.empty() ) return dist( citya, cityz );
		double d = dist( citya, frag[0] );
		REP( i, SIZE(frag)-1 ){
			 d += dist( frag[i], frag[i+1] );
	...
	//	WRITE( tour );
		...
		int repet = 130; // the more repetitons- the more 3-opt operations will be done, and hence, the greater chance, the tour will be shorter
		int A = 12;
		REP(i,repet){
			
			random_shuffle( ALL(perm) );
			REP( k,SIZE(perm) ){
				if( k < (N>>1) ) _pathKOpt( perm[ (k+(N/3))%N ], 4 );			
				_3opt( perm[k],A );	
				
				if( k < (N>>5) ) _pathKOpt( perm[ (k+(2*N/3))%N ],5 );
				
			//	if( k < (N/15) ) _KOpt( perm[ (k+(2*N/3))%N ],4, A+4 );											
			}
...
int getCitiesTime( VI & tour ){
	int t = 0;
	REP( i, SIZE(tour) ){
		t += citytime[ tour[i] ][ i%7 ];
	}
	return t;
}


VI getBestWeeklyTour( VI & tour, double perimeter ){
	VI best = tour;
	double bestres = INF;
	...
	//	cout << "tour:\t"; WRITE(tour);
		...
		REP( i,TSP::N ){
			t = getCitiesTime( tour );
			if( perimeter + t - TSP::dist( tour[0], tour.back() ) < bestres ){
				bestres = perimeter + t - TSP::dist( tour[0], tour.back() );
				best = tour;
				
			//	cout << "Zmieniam best:\t"; WRITE(best);
			}
...
void solve(){
	...
	REP(i,N){
		scanf( "%d %d",&x,&y );
		D[i] = MP( x,y );
		REP(k,7) scanf("%d",&citytime[i][k]  );		
	}
	
	VI tour = TSP::TSP(D);
	
	tour = getBestWeeklyTour( tour, TSP::perimeter );

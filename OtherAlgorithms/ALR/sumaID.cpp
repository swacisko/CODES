...
public:
	...
	void execute(){
		...
		switch( which_case ){
			case 0:{
				if( id == 0 ) sendMessage( neighbours[0], id_los ); 
			...
			case 1:{
				int suma = getLatestMessage( neighbours[1] ).liczba;
				if( suma == notif::BLANK );				
				else if( rnd >= number_of_vertices ){
					suma_id = suma;
					sendMessage( neighbours[0], suma );
				}
				else sendMessage( neighbours[0], suma + id_los );
				
				
				if( ++rnd == 2*number_of_vertices  ) which_case = 3;
				break;
			}
			default:{
				cout << "Koniec dzialania programu, suma_id = " << suma_id << endl;
	...
	void writeState(){
		cout << "Jestem wierzcholkiem o id = " << id << "  id_los = " << id_los <<
			"   runda_nr " << rnd << "  suma_id = " << suma_id << endl;

...
int main(){
	...
	cout << "Podaj rozmiar twoich macierzy:  " << endl;
	int s;
	cin >> s;
	VVD A( s,VD(s) );
	ENDL(2);
	cout << "Podaj wspolczynniki macierzy A" << endl;
	REP( i,s ) REP(k,s) cin >> A[i][k];
	
	cout << "Podaj wspolczynniki macierzy B" << endl;
	VVD B( s,VD(s) );
	ENDL(2);
	REP( i, s) REP( k,s ) cin >> B[i][k];
	
	
	cout << "To twoja macierz A:" << endl;
	write( A );
	ENDL(2);
	cout << "To twoja macierz B:" << endl;
	write( B );
	
	cout << "To macierz A*B:" << endl;
	write( multiply( A,B ) );
	
	
	cout << "Teraz macierz odwrotna do macierzy A" << endl;
	write( B = rev_mat( A ) );
	
	ENDL(2);
	cout << "A teraz iloczyn A i macierzy odwrotnej do niej:" << endl;
	write( multiply( A,B ) );
	
	ENDL(2);
	cout << "A teraz iloscyn macierzy odwrotnej do A oraz A:" << endl;
	write( multiply( B,A ) );
	
	
	ENDL(2);
	cout << "Podaj potege, do ktorej mam podniesc macierz A" << endl;
	int n;
	cin >> n;
	write( power( A,n ) );
	...
	ENDL(2);
	B = A;
	cout << "Wyznacznik macierzy A jest rowny  " << det(A) << endl;
	A = B;
	
	ENDL(2);
	
	cout << "A teraz macierz A w postaci diagonalnej:" << endl;
	diagonalize( A );
	write( A );
	...
	// *********************
/*	cout << "Rozwazmy ciag rekurencyjny:  an(x) = a1(x-1) + a2(x-2) + ... + ak(x-k) + d" << endl;
	cout << "A teraz podaj prosze rozmiar k twojego ciagu rekurencyjnego:  ";
	int K;
	cin >> K;
	VD X( K+1 );
	...
	cin >> X[0];
	REP( i,K ){
		cout << "Podaj prosze wartosc wpolczynnika a" << i+1 << ":  ";
		cin >> X[i+1];
	}
	
	ENDL(2);
	VD beg( K );
	cout << "Teraz podaj prosze warunki pcozatkowe:" << endl;
	REP( i,K ){
		cout << "Wyraz a" << i+1 << " ma byc rowny:   ";
		cin >> beg[i];
	}
	
	ENDL(2);
	cout << "Oto twoje warunki poczatkowe:" << endl;
	WRITE(beg);
	...
	FOR( i,K+1,N ){
		H = solve_eq(X,beg,i);
		cout << "Wyraz " << i << "-ty jest rowny " << H[0] << "  a suma wartosci a1 + ... + a" << i << " = " << H[ SIZE(H)-1 ] << endl;
	}*/
	
	/*cout << "Podaj prosze numer ciagu, ktorego wartosc mam znalezc:   ";
	int n;
	cin >> n;
	X = solve_eq( X,beg,n );
	
	ENDL(1);
	cout << "Wyraz " << n << "-ty jest rowny:  " << X[0] << "  a suma wartosci a1 + ... + an = " << X[0] + X[ SIZE(X)-1 ] << endl;*/

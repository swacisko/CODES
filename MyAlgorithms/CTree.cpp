...
//	typedef int _T; // to co ma byc przechowywane w wezlach
	...
	CTree( int aN ) : N(aN), s(1) {
		...
		FORD( i,s-1,1 ){
			_min[i] = ( _min[(i<<1)].ST <= _min[(i<<1)+1].ST ? _min[(i<<1)] : _min[(i<<1)+1] ); // UWAGA, tutaj od znokow zalezy, index ktorego z elementow o tej samej wartosci zostanie zwrocony!! 
			_max[i] = ( _max[(i<<1)].ST >= _max[(i<<1)+1].ST ? _max[(i<<1)] : _max[(i<<1)+1] ); // jelsi jest <= lub >= to zostanie zwrocony najmniejszy index danewgo ekstremum
		}
	}
	...
	CTree( const vector<_T> & vec ) : N(SIZE(vec)), s(1){
		...
		FORD(i,s-1,1) sum[i] = sum[(i<<1)] + sum[(i<<1)+1]; // przypisuje sume do wszystkich elementow
		REP(i,SIZE(vec)){
	...
	void help_add( int p1, int k1, int g ){
		...
		_min[g] = ( _min[(g<<1)].ST <= _min[(g<<1)+1].ST ? _min[(g<<1)] : _min[(g<<1)+1] ); // UWAGA, tutaj od znokow zalezy, index ktorego z elementow o tej samej wartosci zostanie zwrocony!! 
		_max[g] = ( _max[(g<<1)].ST >= _max[(g<<1)+1].ST ? _max[(g<<1)] : _max[(g<<1)+1] ); // jelsi jest <= lub >= to zostanie zwrocony najmniejszy index danewgo ekstremum
		_min[g].ST += el[g];
		_max[g].ST += el[g];
	}
	...
	PTT help_min( int p1, int k1, int g ){
		...
		PTT minpair = min( help_min( p1,NR,(g<<1) ), help_min( NR + 1, k1, (g<<1)+1 ) ); // biore minimum z podwezlow
		minpair.ST += el[g];
		return minpair;
	}
	...
	PTT help_max( int p1, int k1, int g ){
		...
		PTT maxpair = max( help_max( p1,NR,(g<<1) ), help_max( NR + 1, k1, (g<<1)+1 ) ); // biore minimum z podwezlow
		maxpair.ST += el[g];
		return maxpair;
	}

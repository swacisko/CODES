...
struct KMR{
	VI word; // to jest slowo, dla ktorego buduje slownik slow bazowych
	...
	set<int> zbior; // kazdej literze na poczatku trzeba przypisac identyfikator leksykograficznie
...
namespace SUF{
	...
	void writeSA(){
		...
		REP(i,SIZE(word)){
			cout << "suf["<<i<<"] = " << suf[i] << endl; // " odpowiada suffixowi " << word.substr( suf[i] ) << endl;
		}
...
//************************************ tu jest czesc dotyczaca LCP
	...
	void writeLCP(){
		...
		REP(i,SIZE(lcp)) cout << "lcp["<<i<<"] = " << lcp[i] << endl;// " odpowiada najdluzszemu wspolnemu prefiksowi suffiksow " << word.substr( suf[i] ) << " oraz " << word.substr( suf[i+1] ) << endl;
		cout << endl;
	}
...
public:
		...
		class Node{
		public:
			map<int, Node* > sons; // to jest mapa synow w drzewie. Dla kazdej literki, ktora w danym wezle ma sens jest zwianazy inny wierzcholek reprezentujacy dalsze czesci sufiksu
			Node* par; // to jest wskaznik na rodzica danego wierzcholka
			int p,k; // to jest poczatek i koniec fragmentu slowa word z klasy IntSufTree, dla ktorego jest budowane to drzewo...
		...
		VI word; // to jest slowo, dla ktorego budujemy drzewo sufiksowe, jest ono zakonczone znakiem #
	//	const char end_sign;
...
//		Node* akt; // to jest wskaznik na wierzcholek, w ktorym aktualnie jestem, potrzebne przy tworzeniu drzewa...
		...
		IntSufTree( VI s ) : /*end_sign( (char)('z'+1) )*/ word(s) {
		//	word = s + end_sign;
			initSUF();
		...
		// funkcja wypisujaca odpowiednie tablice drzewa IntSufTree
		void writeSufs(){
			SUF::writeSA();
			SUF::writeLCP();
		}
...
int IntSufTree::countSubwords( Node* wsk ){
	int ile = wsk->length;
...
void IntSufTree::writeTree( Node* wsk, int depth ){
	FOREACH( it, wsk->sons ){
		REP(i,depth) cout << "\t";
...
PII IntSufTree::ncountLongestSubword( int n ){

}

int IntSufTree::helpLongestSubword( Node* wsk, int n ){

}

IntSufTree::Node* IntSufTree::occurs( VI & wzo ){
	Node* wsk=root;
...
int IntSufTree::countOccurs( VI & wzo ){
	Node* wsk = occurs( wzo );
	if( wsk == 0 ) return 0;
	return helpCountOccurs( wsk );
}

int IntSufTree::helpCountOccurs( Node* wsk ){
	int ile = 0;
	if( wsk->isEnd ) ile++;
...
VI IntSufTree::allOccurs( VI& wzo ){
	VI v;
	Node* wsk = occurs( wzo );
	if( wsk == 0 ) return v;
	helpAllOccurs( wsk, v );
...
void IntSufTree::helpAllOccurs( Node* wsk, VI & v ){
...
/*	cout << "Podaj slowo, dla ktorego mam stworzyc tablice SA, LCP, LPF: ";
	...
	*/
	...
	cout << "Podaj dlugosc ciagu, ktory chcesz stworzyc:" << endl;
	int N;
	cin >> N;
	cout << "Podaj ciag intow, dla ktorych mam zbudowac drzewo sufiksowe:  ";
	VI s(N);
	REP(i,N) cin >> s[i];
...
//	ENDL(3);
	...
	while(1){
		cout << "Podaj slowo, ja sprawdze czy wystepuje ono w tekscie:" << endl << "Podaj dlugosc slowa:  ";
		int N; cin >> N;
		s = VI(N);
		REP(i,N) cin >> s[i];
		cout << "Slowo  " << wypisz(s) << ( tree.occurs(s) ? "  wystepuje" : "  nie wystepuje" ) << " w twoim tekscie (" << tree.countOccurs(s) << " razy)" << endl;
		if( tree.occurs(s) ){

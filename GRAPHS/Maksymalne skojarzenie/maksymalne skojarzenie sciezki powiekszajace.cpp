...
namespace MATCH{		
	...
	// korzysta z lematu Swata o grafie pietrowym :)
...
int main(){
	...
	if( t = MATCH::MaxMatch() ) cout << "Wartosc maksymalneg oskoajrzenia:  " << t << endl;
	else cout << "Graf nie jest dwudzielny!" << endl;	
	if( !t ) return 0;
	
	
	REP(i,N) if( MATCH::matching[i] != -1 )cout << "Wierzcholek " << i+1 << " jest skojarzony z " << MATCH::matching[i]+1 << endl;
			else cout << "Wierzcholek " << i+1 << " nie jest skojarzony" << endl;

...
int main(){
	...
	cout << "Podaj prosze rozmiar drzewa:  " << endl;
	cin >> N;
	LCA::V = VVI(N);
	
	REP(i,N-1){
		cout << "Podaj krawedz " << i+1 << ":  ";
		cin >> a >> b;
		LCA::V[a-1].PB(b-1);
		LCA::V[b-1].PB(a-1);
	}
	
	REP(i,SIZE(LCA::V)) sort( ALL( LCA::V[i] ) );
	
	while( true ){
		cout << "Podaj wierzcholek ktory ma byc korzeniem:  ";
		cin >> a;
		if( a < 1 || a > N ){
		...
		int root = a-1;
		LCA::create_CTree( root );
		
		cout << "Aby zadac pytanie wybierz 1" << endl << "Aby zmienic korzen wybierz cos innego..." << endl << endl;
		while( true ){
			cin >> a;
			if( a == 1 ){
				cout << "Podaj dwa wierzcholki, dla ktorych mam znalezc LCA:  ";
				cin >> a >> b;
				cout << "Najblizszy wspolny przodek wierzcholkow " << a << " oraz " << b << " to:  " << LCA::Ask( a-1, b-1 )+1 << endl << endl;
			}
			else break;
		}
